# UX/UI Design Audit Suite — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a Playwright-based screenshot + analysis suite that produces `DESIGN_AUDIT.md` with ~94 screenshots and a structured design sprint input report covering all 47 pages of the HugoHerbots.ai app at mobile (375px) and desktop (1440px) viewports.

**Architecture:** Two scripts: `screenshot-all.ts` uses Playwright/Chromium to visit every `/_dev/{pageName}` route and capture full-page screenshots. `generate-report.ts` scans `src/components/HH/` for Tailwind class patterns and assembles a markdown audit report with embedded screenshots. No backend required — dev preview routes work client-side only.

**Tech Stack:** Playwright (@playwright/test), tsx (already installed), Node.js fs/path, Vite dev server on port 5000.
...
1. `tsx` is available: `npx tsx --version` → should print a version number
2. Vite dev server port: confirmed `5000` in `vite.config.ts`
3. Dev preview routing: confirmed `/_dev/{pageName}` in `src/App.tsx:70`
...
```bash
cd "/Users/stephane/Downloads/HugoHerbots.ai /repo/hugoherbots.com"
...
Expected: Output includes "Chromium ... downloaded to ..." — takes a moment.
...
git commit -m "chore: add @playwright/test for UX audit suite"
```
...
**Files:**
- Create: `scripts/audit/screenshot-all.ts`

**Step 1: Create directory**

```bash
mkdir -p "/Users/stephane/Downloads/HugoHerbots.ai /repo/hugoherbots.com/scripts/audit"
```

**Step 2: Create the screenshot script**

Create `scripts/audit/screenshot-all.ts` with this exact content:
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    await ensureDir(path.join(outDir, vp.name));
  }
...
**Step 3: Verify the script compiles (no runtime yet)**

```bash
cd "/Users/stephane/Downloads/HugoHerbots.ai /repo/hugoherbots.com"
npx tsx --check scripts/audit/screenshot-all.ts 2>&1 || npx tsc --noEmit scripts/audit/screenshot-all.ts 2>&1 | head -20
```

Expected: No errors, or only minor type warnings about `any`.

**Step 4: Commit**

```bash
git add scripts/audit/screenshot-all.ts
git commit -m "feat: add Playwright screenshot audit script for all 47 pages"
```
...
Create `scripts/audit/generate-report.ts` with this exact content:
...
function generateReport(): string {
  ...
  // Files with lowest dark mode coverage (potential dark mode issues)
  const lowDarkMode = perFileAnalysis
    .filter(f => f.darkRatio < 0.05 && f.file.startsWith('Admin'))
    .sort((a, b) => a.darkRatio - b.darkRatio)
    .slice(0, 10);

  // Files with low responsive classes (potential mobile issues)
...
```bash
cd "/Users/stephane/Downloads/HugoHerbots.ai /repo/hugoherbots.com"
npx tsc --noEmit --allowImportingTsExtensions scripts/audit/generate-report.ts 2>&1 | head -20
```

Expected: No fatal errors (minor warnings about implicit `any` are OK).

**Step 3: Commit**

```bash
git add scripts/audit/generate-report.ts
git commit -m "feat: add design audit report generator (Tailwind analysis + markdown output)"
```

---

## Task 4: Update package.json with audit scripts
...
In `package.json`, find the `"scripts"` section (currently has: `dev`, `build`, `lint`, `typecheck`, `test`, `config-lint`) and add:

```json
"audit:screenshot": "tsx scripts/audit/screenshot-all.ts",
"audit:report": "tsx scripts/audit/generate-report.ts",
"audit:full": "npm run audit:screenshot && npm run audit:report"
```
...
"scripts": {
  ...
  "test": "node scripts/config-lint.js",
...
**Step 3: Add screenshots/ and DESIGN_AUDIT.md to .gitignore**

Check if `.gitignore` exists and add:

```
# Design audit output (generated files)
screenshots/
DESIGN_AUDIT.md
```
...
git commit -m "chore: add audit npm scripts and gitignore generated audit output"
```

---

## Task 5: Run the full audit

**Pre-flight: Start the dev server in a separate terminal**
...
**Step 1: Verify dev preview routes work**

Open browser and check: `http://localhost:5000/_dev/dashboard`

Expected: Dashboard renders without login prompt.
...
✅ Done: 94/94 screenshots captured
📋 Summary saved to screenshots/_summary.json
```

If some pages fail, note them and check if `/_dev/{pageName}` works for those pages manually.
...
```bash
ls screenshots/desktop/ | wc -l   # Should be 47
ls screenshots/mobile/ | wc -l    # Should be 47
wc -l DESIGN_AUDIT.md             # Should be 300+ lines
```

**Step 5: Open and review**

Open `DESIGN_AUDIT.md` in a markdown viewer (VS Code preview, Typora, etc.) to verify:
- Screenshots are embedded and display correctly
- All 6 sections present
- Color/typography analysis populated
- Per-page checklists present
...
→ Start `npm run dev` in a separate terminal first.

**Some pages show blank/loading screen**
→ Increase the `waitForTimeout(800)` to `1500` in `screenshot-all.ts` for those pages.

**"/_dev/pageName shows login screen"**
→ The `/_dev/` route only works in DEV mode. Make sure you're running `npm run dev`, not a production build.

**TypeScript errors in scripts**
→ The scripts use `any` types intentionally for Playwright page objects. Add `// @ts-nocheck` at top if needed.

**Screenshot files are too large**
→ Playwright defaults to PNG. For smaller files, change to `{ type: 'jpeg', quality: 85 }` in the screenshot call.
